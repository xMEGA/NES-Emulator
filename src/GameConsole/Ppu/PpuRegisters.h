...
{    
    struct
    {
        ...
        uint8_t BgChrBaseAddr        : 1; // Базовый адрес знакогенератора для формирования фона ( 0 - 0x0000, 1 - 0x1000 )
...
union PpuCtrlReg2_t
{
    struct
    {
        ...
        uint8_t BgVisibleRegion      : 1; // Область видимости фона ( 0 - фон невидим в крайнем левом столбце из 8 точек, 1 - фон присутствует на всем экране )
        ...
        uint8_t BgVisibleEnable      : 1; // Управление отображением фона ( 0 - фон не отображается, 1 - фон отображается )
